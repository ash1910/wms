...
<?php
	...
	{ 
		...
		DB::select("update tbl_acc_masters as t1
					...
					WHERE tbl_acc_masters.type_id > 12 AND tbl_acc_details.ahead <> 'Primary' AND tbl_acc_details.tdate BETWEEN '$From_Date' AND '$To_Date' 
	...
	<div id='printable_div_id' class="card"  style="padding:20px">  <!-- Printing Area Start -->
		...
		<table id="example4" class="table caption-top" >
			...
			@if(isset( $rep_data_ComInfo))
				<caption style="text-align:center"> <font size="4"> <b>{{$rep_data_ComInfo[0]->com_name}}</b> </font> <br> <font size="3">Profit & Loss A/C - {{$myOption}} </font><br>( Period from {{date('d-m-Y', strtotime($From_Date))}} and {{date('d-m-Y', strtotime($To_Date))}} )</caption>
			@endif
				...
				<tbody id="myBody">
					...
					@foreach($categories as $category)
						...
						@if(count($category->childs))			
							@include('acc_manageChild_pl',['childs' => $category->childs])    										
						@endif
