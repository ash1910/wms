...
$dt_Sup_Name = DB::select("SELECT `supplier_name` FROM `suppliers` WHERE `supplier_id` ='$myAccount'");

if (isset($dt_Sup_Name) )
{ 
	$mySup_Name  = $dt_Sup_Name[0]->supplier_name;
}
...
{ 
 ...
 WHERE (`ahead`='Supplier Accounts') AND `others_id` = '$Acc_Name' and tdate < '$From_Date' ");

 $dt_Report_Ledger = DB::select("SELECT * from tbl_acc_details WHERE  (`ahead`='Supplier Accounts')
  AND `others_id` = '$Acc_Name' and tdate BETWEEN '$From_Date' AND '$To_Date' ORDER BY tdate");

$dt_Report_Ledger_total = DB::select("SELECT sum(debit) as debit, sum(credit) as credit FROM `tbl_acc_details` 
			WHERE  (`ahead`='Supplier Accounts') AND `others_id` = '$Acc_Name' and tdate BETWEEN '$From_Date' AND '$To_Date'");
	...
	<div id='printable_div_id' class="card"  style="padding:20px">  <!-- Printing Area Start -->
		...
		<table id="example4" class="table caption-top" >
			...
			@if(isset( $rep_data_ComInfo))
			  <caption style="text-align:center" > <font size="4"> <b>{{$rep_data_ComInfo[0]->com_name}}</b> </font> <br> A/C Name : {{ $Acc_Name}} - {{$mySup_Name}} <br> ( Period from {{date('d-m-Y', strtotime($From_Date))}} and {{date('d-m-Y', strtotime($To_Date))}} )</caption>
			@endif
