...
DB::select("update tbl_acc_masters as t1
							inner join (SELECT
											tbl_acc_masters.id As myid,
											SUM(tbl_acc_details.debit-tbl_acc_details.credit) AS mybal
										FROM tbl_acc_masters
											INNER JOIN tbl_acc_details
											ON tbl_acc_masters.acc_name = tbl_acc_details.ahead
											INNER JOIN tbl_acc_types
											ON tbl_acc_masters.type_id = tbl_acc_types.id
										WHERE tbl_acc_masters.type_id < 13 And tbl_acc_details.ahead <> 'Primary' AND tbl_acc_details.tdate < '$From_Date' 
										GROUP BY tbl_acc_masters.id
										ORDER BY tbl_acc_types.parent_name) as t2
							set t1.op_bal = t2.mybal
							where t1.id = t2.myid;");



DB::select("update tbl_acc_masters as t1
							inner join (SELECT tbl_acc_masters.id As myid, 
										SUM(tbl_acc_details.debit) AS mydebit, SUM(tbl_acc_details.credit) AS mycredit 
										FROM tbl_acc_masters INNER JOIN tbl_acc_details ON tbl_acc_masters.acc_name = tbl_acc_details.ahead 
										WHERE tbl_acc_details.ahead <> 'Primary' AND tbl_acc_details.tdate BETWEEN '$From_Date' AND '$To_Date' 
										GROUP BY tbl_acc_masters.id ORDER BY tbl_acc_masters.id) as t2
							set t1.debit = t2.mydebit, t1.credit = t2.mycredit
							where t1.id = t2.myid;");
...
DB::select("update tbl_acc_masters as t1
 ...
 $rep_data_total = DB::select("SELECT sum(op_bal) as opening, sum(debit) as debit, sum(credit) as credit from tbl_acc_masters where grp_status = 'AH'");
...
<main class="page-content">
	...
	@if(isset( $rep_data_ComInfo))
		<caption style="text-align:center"> <font size="4"> <b>{{$rep_data_ComInfo[0]->com_name}}</b> </font> <br> <font size="3">Trial Balance- {{$myOption}} </font><br>( Period from {{date('d-m-Y', strtotime($From_Date))}} and {{date('d-m-Y', strtotime($To_Date))}} )</caption>
	@endif
	...
	<tbody id="myBody">
		...
		@foreach($categories as $category)
			...
			<tr >
				...
				<td  style="text-align:right"> <b><?php echo number_format($category->debit) ?></b></td>
				<td  style="text-align:right"> <b><?php echo number_format($category->credit) ?></b></td>
				<td  style="text-align:right"> <b><?php echo number_format($category->op_bal+$category->debit-$category->credit) ?></b></td>
			</tr>

			@if(count($category->childs))			
				@include('acc_manageChild',['childs' => $category->childs])    										
			@endif
