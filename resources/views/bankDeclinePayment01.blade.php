             ...
             <div class="card-header py-3">
                  <div class="row align-items-center g-3">
                    <div class="col-12 col-lg-6">
                      <h5 class="mb-0"> {{$reg_no != "" ? "Reg No: ".$reg_no : ""}} {{$chassis_no != "" ? "Chassis No: ".$chassis_no : ""}} | Card No: {{$card_no}}</h5>
                    </div>
                  </div>
             </div>
...
<?php
if( !empty($reg_no)){
  $row = DB::table('customer_info')->where('customer_reg', $reg_no)->where('flag', '1')->first();
}
elseif( !empty($chassis_no)){
  $row = DB::table('customer_info')->where('customer_chas', $chassis_no)->where('flag', '1')->first();
}

$customer_id = $row ? $row->customer_id : "";
$customer_nm = $row ? $row->customer_nm : "";

$result = DB::select("
SELECT `bill` bill_no, a.`job_no`, a.`bill_dt`, a.`net_bill`, `received`, `bonus`, `due`,
`pay_type`, `ref`, `dt`, a.`user_id` , trix,send,bank,chequeNo,chequeDt, a.id, c.user_name,
a.`bonus`, a.`vat_wav`, a.`ait`
FROM `pay` a, user c
WHERE 
1=1
AND a.`customer_id` = '$customer_id' AND a.`card_no` = '$card_no' 
...
?>			  
                  ...
                  <div class="card-body text-left">
                     ...
                     <p class="mb-0"><b>Customer: {{$customer_nm}}</b></p>
                     <p class="mb-0"><b>Bill No: {{$item->bill_no}}</b></p>
...
<div class="row row-cols-1 row-cols-2">		
  ...
  <form class="row g-3" action="bankDeclinePaymentConfirm" method='post' target="_blank">{{ csrf_field() }}
		<div class="col">
		<input type="hidden" name="id" value="{{$item->id}}">
		<input type="hidden" name="bill" value="{{$item->bill_no}}">
		<input type="hidden" name="job_no" value="{{$item->job_no}}">
			<button class="btn btn-sm btn-success me-2" type="submit" name="" value="" style="width: 175px;">
					<i class="fadeIn animated bx bx-printer"></i> Decline Bank(POS)</button>
		</div>
	</form>
