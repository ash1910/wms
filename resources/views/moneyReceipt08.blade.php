...
");
		foreach($result03 as $post03)
			{
				$chequeNo = $post03->chequeNo;
				$job_no = $post03->job_no;
				$customer_id = $post03->customer_id;
				$received = $post03->received;
			}


$result01 = DB::select("
SELECT `id`,`bill`, `job_no`, `customer_id`, `bill_dt`, `net_bill`, `pay_type`,
`dt`, `user_id`,  `trix`, `send`, `bank`, `chequeNo`, `chequeDt`,`charge`,`card_bank`, `card_no`, `card_type`, `merchant_bank`
FROM `pay` WHERE customer_id='$customer_id' and job_no='$job_no' and `pay_type` = 'cheque'
order by id
");
...
<tr>
	<td colspan="3"><b>Received with thanks from </b>: {{$customer_id}}</td>
</tr>
<tr>
	<td>
	<b>Job No.</b>: {{$job_no }}
...
<tr>
	...
	<b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Paid ({{$pay_type}})</b>: Tk.{{number_format(intval($received), 2, '.', ',')}}
	<?php if($pay_type=="cheque"){ echo '(CIH)';}?>
<?php
if($bill=="Advance")
{ ?>
	<b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Advance</b>: Tk.{{number_format(intval(abs($due)), 2, '.', ',')}}
<?php }
if($bill!="Advance")
{ ?>
	<b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Due
		</b>: Tk.{{number_format(intval(abs($due)), 2, '.', ',')}}
<?php } ?>	</td>
</tr>
<?php
if($pay_type=="bkash")
