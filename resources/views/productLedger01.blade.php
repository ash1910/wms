...
a.`purchase_dt`, a.supplier_id, c.supplier_name, b.purchase_id, b.grn
...
AND a.supplier_id = c.supplier_id;
						...
						?>				
						<tr>
							<td style="border: 1px solid black;text-align: center;">{{$item->purchase_dt}}</td>
							<td style="border: 1px solid black;text-align: center;">{{$item->purchase_id}}</td>
							<td style="border: 1px solid black;text-align: center;">{{$item->supplier_id}} - {{$item->supplier_name}}</td>
			...
			<div class="table-responsive">
				<table class="table table-bordered mb-0" style="font-size: 11px;">
					...
					<tbody>				
						...
						SELECT `gin`,`qty`,`avg_price`,`amount`,`job_no`,`dt`  
						FROM `issue` 
