...
$data = DB::select("SELECT b.supplier_name,b.supplier_id, a.amount, a.supplier_ref, a.flag
FROM `purchase_mas` a, suppliers b
WHERE a.`purchase_id` = '$id' AND a.supplier_id= b.supplier_id;");
foreach($data as $item){ $supplier_name = $item->supplier_name ;$amount = $item->amount;
$flag = $item->flag;
$supplier_ref = $item->supplier_ref;$supplier_id = $item->supplier_id;}
...
<?php } ?>						
				...
				<div class="col-7">
                  <div class="">
                    ...
                    <address class="m-t-5 m-b-5">
                       <strong class="text-inverse">Total Amount: Tk. {{number_format(($amount), 2, '.', ',');}}<br>
					   </strong>
                    </address>
                   </div>
                 </div>
              ...
              <th scope="col">Note</th>
					...
					<tbody>
<?php
	$stock = DB::table('purchase_det')->where('purchase_id', $id)->orderBy('id', 'DESC')->get(); 
	$sl = '1';
