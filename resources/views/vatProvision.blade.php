             ...
             <div class="card-header py-3">
                  <div class="row align-items-center g-3">
                    <div class="col-12 col-lg-6">
                      <h5 class="mb-0">VAT Provision List</h5>
                    </div>
				...
				<table id="example2" class="table table-bordered mb-0">
					<thead>
						<tr>
							...
							<th scope="col" style="border: 1px solid black;text-align: center;">Collection</th>
						</tr>
					</thead>
					<tbody>				
<?php
...
?>				
					<tr>
						...
						<td style="border: 1px solid black;text-align: center;">Tk. {{number_format(($item->vat_pro), 2, '.', ',');}}</td>
						<td style="border: 1px solid black;text-align: center;">Tk. {{number_format(($item->total), 2, '.', ',');}}</td>
											
						<td style="border: 1px solid black;text-align: center;">
						
							<!--form class="row g-3" action="ait01" method='post' target="_blank"-->
<?php 
if ((session('role')=="Super Administrator")||(session('role')=="Accounts"))
{
?>					
							<form class="row g-3" action="vat_proApproval" method='post'>
							{{ csrf_field() }}
								<div class="col-12">
								<input type="hidden" name="id" value="{{$item->id}}">
									<button class="btn btn-sm btn-success me-2" type="submit" name="" value="">
											<i class="fadeIn animated bx bx-collection"></i> Collection</button>
								</div>
							</form>	
<?php } else {?>						
							<form class="row g-3" action="vat_proApproval" method='post'>
							{{ csrf_field() }}
								<div class="col-12">
								<input type="hidden" name="id" value="{{$item->id}}">
									<button class="btn btn-sm btn-secondary me-2" type="submit" name="" value="" disabled>
											<i class="fadeIn animated bx bx-collection"></i> Collection</button>
								</div>
							</form>	
<?php } ?>						
